...
import {insertDocument,insertDocumentCharges,insertDocumentItems,insertDocumentTransportation ,updatePartyBalance,updateStockQuantities} from './_helper/db-helper'
// GET /documents
export const getAllDocuments = async (req: Request, res: Response):Promise<any> => {
  try {
    ...
    const {
      documentType, partyId, startDate, endDate,
      status, search, transactionType,
    } = req.query;
    ...
    if (transactionType) { query += ' AND transactionType = ?'; params.push(transactionType); }
    if (search) {
      const s = `%${search}%`;
      query += ' AND (partyName LIKE ? OR documentNumber LIKE ? OR phone LIKE ?)';
      params.push(s, s, s);
    }
    ...
    for (const doc of documents) {
      doc.items = await db.raw('SELECT * FROM document_items WHERE documentId = ?', [doc.id]);
      doc.charges = await db.raw('SELECT * FROM document_charges WHERE documentId = ?', [doc.id]);
      doc.transportation = await db.raw('SELECT * FROM document_transportation WHERE documentId = ?', [doc.id]);
    }
...
export const createDocument = async (req: Request, res: Response):Promise<any> => {
  try {
    ...
    if (!body.documentType || !body.documentDate || !body.partyName || !body.transactionType) {
      return res.status(400).json({
        error: 'documentType, documentDate, partyName, and transactionType are required',
      });
    }
    ...
    const exists = await db.raw(
      'SELECT id FROM documents WHERE firmId = ? AND documentNumber = ?',
      [firmId, body.documentNumber]
    );
    ...
    await insertDocumentTransportation(body.transportation || [], documentId, firmId);
    await updateStockQuantities(body.documentType, body.items || [], firmId);
    await updatePartyBalance(body, firmId);

    const created = await db.raw('SELECT * FROM documents WHERE id = ?', [documentId]);
    if (created.length === 0) return res.status(500).json({ error: 'Document created but not retrievable' });

    const doc = created[0];
    doc.items = await db.raw('SELECT * FROM document_items WHERE documentId = ?', [documentId]);
    doc.charges = await db.raw('SELECT * FROM document_charges WHERE documentId = ?', [documentId]);
    doc.transportation = await db.raw('SELECT * FROM document_transportation WHERE documentId = ?', [documentId]);
...
export const getDocumentById = async (req: Request, res: Response):Promise<any> => {
  try {
    ...
    const doc = await db.raw('SELECT * FROM documents WHERE id = ? AND firmId = ?', [id, firmId]);
    if (!doc || doc.length === 0) return res.status(404).json({ error: 'Document not found' });

    const [base] = doc;

    base.items = await db.raw('SELECT * FROM document_items WHERE documentId = ? AND firmId = ?', [id, firmId]);
    base.charges = await db.raw('SELECT * FROM document_charges WHERE documentId = ? AND firmId = ?', [id, firmId]);
    base.transportation = await db.raw('SELECT * FROM document_transportation WHERE documentId = ? AND firmId = ?', [id, firmId]);
    ...
    base.relationships = {
      sourceDocuments: await db.raw(`
        ...
        WHERE dr.sourceDocumentId = ? AND dr.firmId = ?`, [id, firmId]),
      targetDocuments: await db.raw(`
        SELECT dr.*, d.documentType, d.documentNumber, d.documentDate, d.status
        FROM document_relationships dr
        JOIN documents d ON dr.sourceDocumentId = d.id
        WHERE dr.targetDocumentId = ? AND dr.firmId = ?`, [id, firmId]),
    };
...
export const updateDocument = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const existingDoc = await db.raw('SELECT * FROM documents WHERE id = ? AND firmId = ?', [id, firmId]);
    ...
    const oldItems = await db.raw('SELECT * FROM document_items WHERE documentId = ? AND firmId = ?', [id, firmId]);
    
    // If document number is changed, check for duplicates
    if (body.documentNumber && body.documentNumber !== oldDoc.documentNumber) {
      const exists = await db.raw(
        'SELECT id FROM documents WHERE firmId = ? AND documentNumber = ? AND id != ?',
        [firmId, body.documentNumber, id]
      );
    ...
    const { items, charges, transportation, relationships, ...docFields } = body;
    ...
    await db.raw(`UPDATE documents SET ${updateSql} WHERE id = ? AND firmId = ?`, values);
    ...
    const updated = await db.raw('SELECT * FROM documents WHERE id = ? AND firmId = ?', [id, firmId]);
    if (!updated || updated.length === 0) {
      return res.status(500).json({ error: 'Document updated but not retrievable' });
    }
    
    const finalDoc = updated[0];
    finalDoc.items = await db.raw('SELECT * FROM document_items WHERE documentId = ? AND firmId = ?', [id, firmId]);
    finalDoc.charges = await db.raw('SELECT * FROM document_charges WHERE documentId = ? AND firmId = ?', [id, firmId]);
    finalDoc.transportation = await db.raw('SELECT * FROM document_transportation WHERE documentId = ? AND firmId = ?', [id, firmId]);
