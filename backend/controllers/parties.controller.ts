...
export const getAllParties = async (req: Request, res: Response):Promise<any> => {
  try {
    ...
    const partiesWithFields = parties.map(party => {
      ...
      return {
        ...party,
        additionalFields: fields.map(f => ({ key: f.fieldKey, value: f.fieldValue })),
...
export const createParty = async (req: Request, res: Response):Promise<any> => {
  try {
    const firmId = req.headers['x-firm-id'] as string || '';
    const { additionalFields, ...partyData } = req.body;
...
export const getPartyById = async (req: Request, res: Response):Promise<any> => {
  try {
    const firmId = req.headers['x-firm-id'] as string || '';
    const { id } = req.params;
    ...
    const formattedFields = additionalFields.map(f => ({ key: f.fieldKey, value: f.fieldValue }));
...
export const updateParty = async (req: Request, res: Response):Promise<any> => {
  try {
    const firmId = req.headers['x-firm-id'] as string || '';
    const { id } = req.params;
    const { additionalFields, ...updateData } = req.body;

    const existingParty = await db('parties', firmId).where('id', id).first();
    if (!existingParty) return res.status(404).json({ success: false, error: 'Party not found' });
    ...
    const formattedFields = updatedFields.map(f => ({ key: f.fieldKey, value: f.fieldValue }));
    ['shippingEnabled', 'paymentReminderEnabled', 'loyaltyPointsEnabled'].forEach(field => {
      if (field in updatedParty) updatedParty[field] = Boolean(updatedParty[field]);
    });
...
export const deleteParty = async (req: Request, res: Response):Promise<any> => {
  try {
    const firmId = req.headers['x-firm-id'] as string || '';
    const { id } = req.params;
