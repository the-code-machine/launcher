...
import { useDeleteDocumentMutation, useGetDocumentsQuery } from '@/redux/api/documentApi'
...
const DocumentListView: React.FC<DocumentListProps> = ({ 
  ...
  const { 
    ...
    documentType: documentType ? convertDocumentTypeToApiParam(documentType) : 'sale_invoice',
  ...
  const filteredDocuments = documents?.filter((document) => {
    const matchesSearch = (
      (document.partyName?.toLowerCase() || '').includes(filterDocument.toLowerCase()) ||
      (document.documentNumber?.toLowerCase() || '').includes(filterDocument.toLowerCase())
    )

    if (currentTab === 'all') return matchesSearch
    if (currentTab === 'cash' && document.transactionType === 'cash') return matchesSearch
    if (currentTab === 'credit' && document.transactionType === 'credit') return matchesSearch
    if (currentTab === 'due' && document.balanceAmount > 0) return matchesSearch
  ...
  const mainDocInfo = documentType ? getDocumentInfo(documentType) : { title: 'Documents' };
                    ...
                    filteredDocuments?.map((document) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant={document.transactionType === 'cash' ? 'outline' : 'secondary'}
                              className={document.transactionType === 'cash' ? 'bg-green-50 text-green-700 border-green-200' : ''}
                            >
                              {document.transactionType === 'cash' ? 'Cash' : 'Credit'}
                            </Badge>
                          </TableCell>
                                  ...
                                  {docInfo.convertOptions.length > 0 && (
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent align="start" className="w-48">
                                        {docInfo.convertOptions.map((option, idx) => (
                                          <DropdownMenuItem 
                                            key={idx}
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleConvertDocument(document.id, option.type);
                                            }}
                                          >
                                            {option.label}
                                          </DropdownMenuItem>
            ...
            <div className="flex items-center justify-between">
              <CardTitle>
                {selectedDocument ? (
                  <div className="flex items-center gap-2">
                    ...
                    {getDocumentInfo(selectedDocument.documentType).title} Details
                    <Badge
                      variant={selectedDocument.transactionType === 'cash' ? 'outline' : 'secondary'}
                      className={selectedDocument.transactionType === 'cash' ? 'bg-green-50 text-green-700 border-green-200' : ''}
                    >
                      {selectedDocument.transactionType === 'cash' ? 'Cash' : 'Credit'}
                    </Badge>
                  </div>
                ) : (
                  'Document Details'
              ...
              {selectedDocument && (
                <div className="flex gap-2">
                  ...
                  <Button 
                    size="sm"
                    onClick={() => router.push(`/viewer?Id=${selectedDocument.id}`)}
                  >
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDate(selectedDocument.documentDate)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">{selectedDocument.partyType === 'customer' ? 'Customer' : 'Supplier'}</p>
                    <p className="text-sm font-medium">{selectedDocument.partyName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Phone</p>
                    <p className="text-sm font-medium">{selectedDocument.phone || '—'}</p>
                  </div>
                </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <p className="text-lg font-semibold">{formatCurrency(selectedDocument.total)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Payment Type</p>
                        <p className="text-lg font-semibold capitalize">{selectedDocument.paymentType}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">{selectedDocument.partyType === 'customer' ? 'Received' : 'Paid'} Amount</p>
                        <p className="text-lg font-semibold text-green-600">{formatCurrency(selectedDocument.paidAmount)}</p>
                      </div>
                     { selectedDocument.documentType != DocumentType.SALE_QUOTATION && <div>
                        <p className="text-xs text-muted-foreground">Balance Due</p>
                        <p className={`text-lg font-semibold ${selectedDocument.balanceAmount > 0 ? 'text-orange-600' : 'text-green-600'}`}>
                ...
                <div className="space-y-2">
                      ...
                      <TableBody>
                        {selectedDocument.items?.map((item, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell className="text-right">{item.primaryQuantity} {item.primaryUnitName}</TableCell>
                  ...
                  {/* Additional costs */}
              { selectedDocument &&   <div className="pt-2 space-y-1 border-t">
                    { (selectedDocument.discountAmount ?? 0) > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Discount</span>
                        <span>-{formatCurrency(Number(selectedDocument.discountAmount))}</span>
                      </div>
                    )}
                    {selectedDocument.taxAmount && selectedDocument.taxAmount > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Tax</span>
                        <span>+{formatCurrency(selectedDocument.taxAmount || 0)}</span>
                      </div>
                    )}
                    {selectedDocument.shipping && selectedDocument.shipping > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Shipping</span>
                        <span>+{formatCurrency(selectedDocument.shipping || 0)}</span>
                      </div>
                    )}
                    {selectedDocument.packaging && selectedDocument.packaging > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Packaging</span>
                        <span>+{formatCurrency(selectedDocument.packaging || 0)}</span>
                      </div>
                    )}
                    {selectedDocument.roundOff !== 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Round Off</span>
                        <span>{selectedDocument.roundOff > 0 ? '+' : ''}{formatCurrency(selectedDocument.roundOff)}</span>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        {selectedDocument.transportName && (
                          <div>
                            ...
                            <p className="text-sm font-medium">{selectedDocument.transportName}</p>
                          </div>
                        )}
                        {selectedDocument.vehicleNumber && (
                          <div>
                            <p className="text-xs text-muted-foreground">Vehicle Number</p>
                            <p className="text-sm font-medium">{selectedDocument.vehicleNumber}</p>
                          </div>
                        )}
                        {selectedDocument.deliveryDate && (
                          <div>
                            <p className="text-xs text-muted-foreground">Delivery Date</p>
                            <p className="text-sm font-medium">{formatDate(selectedDocument.deliveryDate)}</p>
                          </div>
                        )}
                        {selectedDocument.deliveryLocation && (
                          <div>
                            <p className="text-xs text-muted-foreground">Delivery Location</p>
                            <p className="text-sm font-medium">{selectedDocument.deliveryLocation}</p>
                          </div>
                        )}
                      </div>
                    </CardContent>
                ...
                {getDocumentInfo(selectedDocument.documentType).convertOptions.length > 0 && (
                  <div className="pt-4 border-t">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {getDocumentInfo(selectedDocument.documentType).convertOptions.map((option, idx) => (
                        <Button 
                          key={idx} 
                          variant="outline"
                          size="sm"
                          onClick={() => handleConvertDocument(selectedDocument.id, option.type)}
                        >
                          {option.label}
                        </Button>
                      ))}
