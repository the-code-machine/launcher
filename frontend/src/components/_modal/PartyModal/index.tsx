...
const AddParty = () => {
  ...
  const [createParty, { isLoading: isCreatingParty }] =
    useCreatePartyMutation();
  const [updateParty, { isLoading: isUpdatingParty }] =
    useUpdatePartyMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dispatch(setSubmitError(error.data.error || "Failed to save party"));
      toast.error(
        `Failed to save party: ${error.data.error || "Unknown error"}`
      );
    } finally {
                    ...
                    <div className="pt-4">
                      ...
                      formData.additionalFields.length > 0 ? (
                          ...
                          {formData.additionalFields.map((field, index) => (
                            <div key={index} className="flex items-start gap-2">
                              ...
                              <Button
                                ...
                                size="icon"
                                onClick={() =>
                                  handleRemoveAdditionalField(index)
                                }
                              >
                        ...
                        <p className="text-sm text-muted-foreground text-center py-4">
                          No additional fields. Click Add Field to create custom
                          fields.
                        </p>
                      )}
                    </div>
