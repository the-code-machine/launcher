...
const PurchasesReport = () => {
  ...
  const calculateTotals = () => {
    if (!purchaseInvoices) return { count: 0, total: 0, paid: 0, balance: 0 }
    
    const total = purchaseInvoices.reduce((sum, invoice) => sum + invoice.total, 0)
    const paid = purchaseInvoices.reduce((sum, invoice) => sum + invoice.paidAmount, 0)
    const balance = purchaseInvoices.reduce((sum, invoice) => sum + invoice.balanceAmount, 0)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <DownloadButton buttonText='Export XlSX'  data={purchaseInvoices|| []} fileName="sales-report" />
          <Button 
            variant="outline" 
            size="sm"
            className="flex items-center gap-1"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden">
        ...
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-4">
            ...
            <h3 className="text-2xl font-bold mt-1 text-green-600">₹{totals.paid.toFixed(2)}</h3>
          </CardContent>
        </Card>
      ...
      <Card className="print:shadow-none print:border-none">
        <CardHeader className="pb-3 print:hidden">
          <CardTitle className="text-md flex items-center">
            <ShoppingBagIcon className="h-4 w-4 mr-2 text-primary" />
            Purchase Invoices
          </CardTitle>
        </CardHeader>
        ...
        <CardContent>
          ...
          <div ref={printRef} className="hidden print:block">
            ...
            <div className="text-center mb-4">
              ...
              <h3 className="text-lg font-semibold mt-4 border-b-2 border-gray-300 pb-2">Purchase Report</h3>
            </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {!isLoading && !isError && purchaseInvoices?.map((invoice) => (
                  <tr key={invoice.id}>
            ...
            <div className="text-right mt-4">
              <p className="font-semibold">Total Purchase: ₹ {totals.total.toFixed(2)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
