...
const LowStockAlertPage = () => {
  ...
  const { data: items, isLoading: isLoadingItems, isError: isItemsError, refetch: refetchItems } = useGetItemsQuery({ 
    type: 'PRODUCT', // Only get products (not services)
  ...
  const calculateSeverity = (item: any) => {
    if (!isProduct(item) || item.minStockLevel === undefined || item.currentQuantity === undefined) {
      return null;
    }
    ...
    return items
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          ...
          case "percentage":
            const aPercent = (a.currentQuantity || 0) / (a.minStockLevel || 1) * 100;
            const bPercent = (b.currentQuantity || 0) / (b.minStockLevel || 1) * 100;
  ...
  const purchaseOrderSuggestions = React.useMemo(() => {
    ...
    lowStockItems.forEach(item => {
      ...
      const orderQuantity = Math.max((item.minStockLevel ?? 0) - (item.currentQuantity ?? 0), 0);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="quantity">Current Quantity (Lowest First)</SelectItem>
                  <SelectItem value="percentage">Stock Percentage (Lowest First)</SelectItem>
                  <SelectItem value="days">Days to Reorder (Lowest First)</SelectItem>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden">
        <Card className={`border-red-200 ${lowStockItems.filter(item => item.severity === 'critical').length > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-500">Critical</p>
                <h3 className="text-2xl font-bold mt-1 text-red-600">{severityCounts.critical}</h3>
        ...
        <Card className={`border-amber-200 ${lowStockItems.filter(item => item.severity === 'warning').length > 0 ? 'bg-amber-50' : 'bg-gray-50'}`}>
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-500">Warning</p>
                <h3 className="text-2xl font-bold mt-1 text-amber-600">{severityCounts.warning}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  Reorder soon
                </p>
              </div>
        ...
        <Card className={`border-blue-200 ${lowStockItems.filter(item => item.severity === 'notice').length > 0 ? 'bg-blue-50' : 'bg-gray-50'}`}>
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-500">Notice</p>
                <h3 className="text-2xl font-bold mt-1 text-blue-600">{severityCounts.notice}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  Monitor closely
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-slate-600">{severityCounts.total}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  All low stock items
                </p>
              </div>
      ...
      <Tabs defaultValue="alerts" className="print:hidden" value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-md">Alert Severity Distribution</CardTitle>
              </CardHeader>
              <CardContent>
                {isLoadingItems ? (
                ...
                {isLoadingItems ? (
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    ...
                    >
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="value" name="Number of Items" fill="#8884d8" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
              ...
              {isLoadingItems ? (
                ...
                <div className="text-center py-8 border rounded-md">
                  ...
                  <p className="text-sm text-gray-500 max-w-md mx-auto mt-1">
                    All your items have sufficient stock levels. You can adjust the filters or minimum stock levels if needed.
                  </p>
                </div>
              ) : (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-center">Est. Days Left</TableHead>
                        <TableHead className="text-center w-[120px]">Severity</TableHead>
                        <TableHead className="text-right">Action</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {lowStockItems.map((item) => {
                        const stockPercentage = ((item.currentQuantity || 0) / (item.minStockLevel || 1)) * 100;
                        ...
                        return (
                          <TableRow key={item.id}>
                            <TableCell className="font-medium">
                              {item.name}
                              {item.itemCode && (
                                <div className="text-xs text-gray-500 mt-1">Code: {item.itemCode}</div>
                              )}
                            </TableCell>
                                ...
                                <div className="w-16 bg-gray-200 rounded-full h-1.5 mt-1">
                                  <div 
                                    className={`h-1.5 rounded-full ${
                                      ...
                                      stockPercentage <= 50 ? 'bg-amber-600' :
                                      'bg-blue-600'
                                    }`}
                            ...
                            <TableCell className="text-center">
                              {item.daysToReorder !== null ? (
                                <Badge className={`
                                  ${item.daysToReorder <= 3 ? 'bg-red-100 text-red-800' : 
                                   item.daysToReorder <= 7 ? 'bg-amber-100 text-amber-800' : 
                            ...
                            <TableCell className="text-center">
                              <Badge className={`capitalize ${SEVERITY_COLORS[item.severity as keyof typeof SEVERITY_COLORS]}`}>
                                {item.severity}
                              </Badge>
                            </TableCell>
        ...
        <TabsContent value="orders" className="space-y-6">
          {isLoadingItems ? (
            ...
            <div className="text-center py-12 border rounded-md">
              ...
              <h3 className="text-lg font-medium text-gray-900">No purchase orders needed</h3>
              <p className="text-sm text-gray-500 max-w-md mx-auto mt-1">
                      ...
                      <TableBody>
                        {suggestion.items.map((item:any, idx:any ) => (
                          <TableRow key={idx}>
                            ...
                            <TableCell className="text-center">{item.currentStock.toFixed(2)}</TableCell>
                            <TableCell className="text-center">{item.minStock.toFixed(2)}</TableCell>
                            <TableCell className="text-center font-medium">{item.orderQuantity.toFixed(2)}</TableCell>
      ...
      <div ref={printRef} className="hidden print:block">
        ...
        <div className="text-center mb-6">
          ...
          <h3 className="text-lg font-semibold mt-4 border-b-2 border-gray-300 pb-2">Low Stock Alert Report</h3>
        ...
        <div className="mb-8 border border-gray-300 p-4 bg-gray-50">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p><strong>Critical Items:</strong> {severityCounts.critical}</p>
            </div>
            <div>
              <p><strong>Warning Items:</strong> {severityCounts.warning}</p>
            </div>
            <div>
              <p><strong>Notice Items:</strong> {severityCounts.notice}</p>
            </div>
          </div>
          <p className="mt-3"><strong>Total Low Stock Items:</strong> {severityCounts.total}</p>
        </div>
        
        {/* Low Stock Items Table */}
        <div className="mb-6">
          <h4 className="text-base font-medium mb-3 border-b border-gray-300 pb-2">Low Stock Items</h4>
          <table className="w-full border-collapse mb-4">
            <thead>
              <tr className="bg-gray-200">
                ...
                <th className="border border-gray-300 p-2 text-center">Min. Stock</th>
                <th className="border border-gray-300 p-2 text-center">Stock %</th>
                <th className="border border-gray-300 p-2 text-center">Est. Days Left</th>
              ...
              {lowStockItems.map((item, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-300 p-2 text-center">
                      {item.daysToReorder !== null ? `${item.daysToReorder} days` : 'N/A'}
                    </td>
                    <td className="border border-gray-300 p-2 text-center">
                      {item.severity}
                    </td>
        ...
        <div className="mb-6">
          <h4 className="text-base font-medium mb-3 border-b border-gray-300 pb-2">Purchase Order Suggestions</h4>
          {purchaseOrderSuggestions.length === 0 ? (
            <p className="text-center py-4">No purchase orders needed at this time.</p>
          ) : (
            purchaseOrderSuggestions.map((suggestion, index) => (
              <div key={index} className="mb-6">
                <h5 className="text-sm font-medium mb-2">Suggested Order: {suggestion.supplier}</h5>
                <table className="w-full border-collapse mb-4">
                  <thead>
                    <tr className="bg-gray-200">
                      <th className="border border-gray-300 p-2 text-left">Item</th>
                      <th className="border border-gray-300 p-2 text-center">Current Stock</th>
                      <th className="border border-gray-300 p-2 text-center">Min. Stock</th>
                      <th className="border border-gray-300 p-2 text-center">Order Quantity</th>
                  ...
                  <tbody>
                    {suggestion.items.map((item:any, idx:any) => (
                      <tr key={idx}>
                        ...
                        <td className="border border-gray-300 p-2 text-center">{item.currentStock.toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-center">{item.minStock.toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-center font-medium">{item.orderQuantity.toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-center">{item.unitName}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="mt-8 p-3 border border-gray-300 bg-gray-50 text-sm">
          <p><strong>Note:</strong> This report shows items that have fallen below their minimum stock levels. 
          Items are categorized by severity based on the percentage of current stock compared to minimum stock:
          Critical (&lt;25%), Warning (25-50%), Notice (50-75%).</p>
        </div>
      </div>
