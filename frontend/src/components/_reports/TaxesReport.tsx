...
const TaxReport = () => {
  ...
  // Filter and process data to get Tax information
  const processTaxData = () => {
    const partyTaxMap = new Map();

    // Process Sale Invoices - Tax In (Output VAT)
      ...
      saleInvoices.forEach((invoice) => {
        ...
        invoice.items &&
          invoice.items.forEach((item) => {
            const taxAmount =
              parseFloat(item.taxAmount?.toString() || "0") || 0;
    ...
    // Process Purchase Invoices - Tax Out (Input VAT)
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:hidden">
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-500">Output VAT (Sales)</p>
                <h3 className="text-2xl font-bold mt-1 text-green-600">
                  ₹{summary.totalTaxIn.toFixed(2)}
                </h3>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-500">Input VAT (Purchases)</p>
                <h3 className="text-2xl font-bold mt-1 text-blue-600">
                  ₹{summary.totalTaxOut.toFixed(2)}
                </h3>
                <p className="text-xs text-gray-500 mt-1">
        ...
        <CardContent>
                    ...
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell className="text-right">
                          {(summary.totalSales - summary.totalTaxIn).toFixed(2)}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">
                      ...
                      <TableRow>
                        ...
                        <TableCell className="text-right">
                          {(
                            summary.totalPurchases - summary.totalTaxOut
                          ).toFixed(2)}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {taxData.length > 0 ? (
                        taxData
                          .sort(
                            (a, b) => b.taxIn + b.taxOut - (a.taxIn + a.taxOut)
                          )
                          .map((party, index) => {
                            ...
                            return (
                              <TableRow key={index}>
                                ...
                                <TableCell className="text-right">
                                  {party.saleTotal.toFixed(2)}
                                </TableCell>
                                <TableCell className="text-right text-green-600">
                                  {party.taxIn.toFixed(2)}
                                </TableCell>
                                <TableCell className="text-right">
                                  {party.purchaseTotal.toFixed(2)}
                                </TableCell>
                                <TableCell className="text-right text-blue-600">
                                  {party.taxOut.toFixed(2)}
                                </TableCell>
                                <TableCell
          ...
          <div ref={printRef} className="hidden print:block">
            ...
            <div className="mb-6 border border-gray-300 p-4 bg-gray-50">
              <h4 className="text-base font-medium mb-3">VAT Summary</h4>
              <table className="w-full">
                <tbody>
                  <tr>
                    <td width="40%">
                      <strong>Total Taxable Sales:</strong>
                    </td>
                  ...
                  <tr>
                    <td>
                      <strong>Output VAT (on Sales):</strong>
                    </td>
                  ...
                  <tr>
                    <td>
                      <strong>Total Taxable Purchases:</strong>
                    </td>
                    <td className="text-right">
                      ₹
                      {(summary.totalPurchases - summary.totalTaxOut).toFixed(
                        2
                      )}
                    </td>
                    <td></td>
                  ...
                  <tr>
                    <td>
                      <strong>Input VAT (on Purchases):</strong>
                    </td>
                    <td className="text-right">
                      ₹{summary.totalTaxOut.toFixed(2)}
                    </td>
            ...
            <table className="w-full border-collapse mb-6">
              ...
              <tbody>
                {!isLoading && !isError && taxData.length > 0 ? (
                  taxData
                    .sort((a, b) => b.taxIn + b.taxOut - (a.taxIn + a.taxOut))
                    .map((party, index) => {
                      const netTax = party.taxIn - party.taxOut;
                      return (
                        <tr key={index}>
            ...
            <div className="mt-6 p-3 border border-gray-300 bg-gray-50 text-sm">
              <p>
                <strong>Note:</strong> This report shows the VAT amounts
                collected from sales (Output VAT) and paid on purchases (Input
                VAT). The difference between Output VAT and Input VAT is either
                payable to the government {`(if Output > Input)`} or can be
                claimed as credit {`(if Input > Output)`}.
              </p>
            </div>
          </div>
        </CardContent>
