...
const StockDetailsPage = () => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    let filtered = items.filter(item => {
      ...
      const isLowStockProduct = lowStockOnly && isProduct(item) ? 
        item.minStockLevel && item.currentQuantity < item.minStockLevel : true;
      
      // Filter by category if selected
      const matchesCategory = !categoryId || item.categoryId === categoryId;
      
      return matchesSearch && isLowStockProduct && matchesCategory && isProduct(item);
    });
    ...
    return filtered.sort((a, b) => {
      ...
      switch(sortBy) {
        ...
        case "quantity":
          ...
          break;
        case "value":
          const valueA = isProduct(a) ? (a.currentQuantity || 0) * (a.salePrice || 0) : 0;
          const valueB = isProduct(b) ? (b.currentQuantity || 0) * (b.salePrice || 0) : 0;
  ...
  const conversionMap = React.useMemo(() => {
    return unitConversions?.reduce((acc, conv) => {
      acc[conv.id] = conv;
      return acc;
    }, {} as Record<string, any>) || {};
  }, [unitConversions]);
  ...
  const stockValues = React.useMemo(() => {
    if (!filteredItems) return { totalStockValue: 0, totalItems: 0, lowStockItems: 0, categories: 0 };
    ...
    const lowStockItems = filteredItems.filter(item => {
      return isProduct(item) && item.minStockLevel && item.currentQuantity < item.minStockLevel;
    }).length;
    
    const uniqueCategories = new Set(filteredItems.map(item => item.categoryId));
  ...
  const getUnitNames = React.useCallback((item: any) => {
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-blue-600">₹{stockValues.totalStockValue.toFixed(2)}</h3>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-green-600">{stockValues.categories}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  Product categories
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-purple-600">{stockValues.lowStockItems}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  Need attention
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1 text-amber-600">{stockValues.totalItems}</h3>
                <p className="text-xs text-gray-500 mt-1">
                  In inventory
                </p>
              </div>
        ...
        <CardContent>
                ...
                <TableBody>
                  {filteredItems.length > 0 ? (
                    filteredItems.map((item) => {
                      ...
                      const stockValue = (item.currentQuantity || 0) * (item.salePrice || 0);
                      const isLowStock = item.minStockLevel && (item.currentQuantity || 0) < item.minStockLevel;
                      ...
                      return (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="text-right">{(item.currentQuantity || 0).toFixed(2)}</TableCell>
                          <TableCell className="text-center">
                            {unitInfo.primary}
                            {unitInfo.conversionRate > 0 && (
                              <div className="text-xs text-gray-500">
                                1 {unitInfo.primary} = {unitInfo.conversionRate} {unitInfo.secondary}
                              </div>
                            )}
                          </TableCell>
                          <TableCell className="text-right">₹{(item.salePrice || 0).toFixed(2)}</TableCell>
          ...
          <div ref={printRef} className="hidden print:block">
            ...
            <div className="text-center mb-4">
              ...
              <h3 className="text-lg font-semibold mt-4 border-b-2 border-gray-300 pb-2">Stock Details Report</h3>
            ...
            <div className="mb-6 border border-gray-300 p-4 bg-gray-50">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div>
                  <p><strong>Total Stock Value:</strong> ₹{stockValues.totalStockValue.toFixed(2)}</p>
                </div>
                <div>
                  <p><strong>Total Products:</strong> {stockValues.totalItems}</p>
                </div>
                <div>
                  <p><strong>Low Stock Items:</strong> {stockValues.lowStockItems}</p>
                </div>
                <div>
                  <p><strong>Categories:</strong> {stockValues.categories}</p>
                </div>
              </div>
            </div>
            ...
            <table className="w-full border-collapse mb-6">
              <thead>
                <tr className="bg-gray-200">
                  ...
                  <th className="border border-gray-300 p-2 text-right">Purchase Price (₹)</th>
                  <th className="border border-gray-300 p-2 text-right">Stock Value (₹)</th>
              ...
              <tbody>
                {!isLoadingItems && !isItemsError && filteredItems.length > 0 ? (
                  filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        <td className="border border-gray-300 p-2">{item.itemCode || 'N/A'}</td>
                        <td className="border border-gray-300 p-2 font-medium">{item.name}</td>
                        <td className="border border-gray-300 p-2">{category?.name || 'N/A'}</td>
                        <td className="border border-gray-300 p-2 text-right">{(item.currentQuantity || 0).toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-center">{unitInfo.primary}</td>
                        <td className="border border-gray-300 p-2 text-right">{(item.salePrice || 0).toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-right">{(item.purchasePrice || 0).toFixed(2)}</td>
                        <td className="border border-gray-300 p-2 text-right">{stockValue.toFixed(2)}</td>
                  ...
                  <tr>
                    <td colSpan={9} className="border border-gray-300 p-2 text-center">No stock items found</td>
                  </tr>
                )}
              </tbody>
              <tfoot>
                <tr className="bg-gray-100 font-bold">
                  ...
                  <td className="border border-gray-300 p-2 text-right">₹{stockValues.totalStockValue.toFixed(2)}</td>
            ...
            <div className="mt-6 p-3 border border-gray-300 bg-gray-50 text-sm">
              <p><strong>Note:</strong> This report shows detailed inventory levels and valuation for all products in stock. 
              Stock value is calculated based on the current quantity and sale price of each item. 
              Items marked as Low Stock have current quantities below their defined minimum stock levels.</p>
            </div>
          </div>
        </CardContent>
      </Card>
