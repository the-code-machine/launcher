    ...
    saleInvoices.forEach((invoice) => {
      invoice.items &&
        invoice.items.forEach((item) => {
          ...
          // Determine tax category (Standard VAT or other types)
          const taxCategory = taxType.includes("VAT") ? "VAT" : "Tax";
          ...
          // Classify tax as standard or reduced rate
          if (rate >= 15) {
            // Assuming rates 15% and above are standard rates
            data.standardTax += taxAmount;
          } else {
            data.reducedTax += taxAmount;
          }
  ...
  const calculateTotals = (data: any) => {
    return data.reduce(
      (acc: any, item: any) => {
        ...
        acc.standardTax += item.standardTax;
        acc.reducedTax += item.reducedTax;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold">Tax Rate Report</h1>
      ...
      <Card className="print:shadow-none print:border-none">
        ...
        <CardContent>
            ...
            <div className="print:hidden">
              ...
              >
                <TabsList className="mb-4">
                  <TabsTrigger value="sales">Sales VAT Rates</TabsTrigger>
                      ...
                      {salesTaxByRate.length > 0 ? (
                        salesTaxByRate.map((item, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell className="text-right">
                              ₹{item.standardTax.toFixed(2)}
                            </TableCell>
                            <TableCell className="text-right">
                              ₹{item.reducedTax.toFixed(2)}
                            </TableCell>
                        ...
                        <TableRow>
                          ...
                          >
                            No VAT sales data found for the selected period
                          </TableCell>
                        </TableRow>
                      )}
                      {salesTaxByRate.length > 0 && (
                        <TableRow className="bg-gray-50 font-medium">
                          ...
                          <TableCell className="text-right">
                            ₹{salesTotals.taxableValue.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{salesTotals.standardTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{salesTotals.reducedTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{salesTotals.totalTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{salesTotals.totalValue.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-center">
                      ...
                      {purchasesTaxByRate.length > 0 ? (
                        ...
                        <TableRow>
                          <TableCell
                            ...
                            className="text-center py-4 text-gray-500"
                          >
                            No VAT purchase data found for the selected period
                          </TableCell>
                        </TableRow>
                      )}
                      {purchasesTaxByRate.length > 0 && (
                        <TableRow className="bg-gray-50 font-medium">
                          <TableCell colSpan={2}>Total</TableCell>
                          <TableCell className="text-right">
                            ₹{purchasesTotals.taxableValue.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{purchasesTotals.standardTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{purchasesTotals.reducedTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{purchasesTotals.totalTax.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right">
                            ₹{purchasesTotals.totalValue.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-center">
                            {purchasesTotals.transactions}
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
          ...
          <div ref={printRef} className="hidden print:block">
            ...
            {/* Period */}
            <p className="mb-4">
              Period: {format(new Date(startDate), "dd/MM/yyyy")} to{" "}
              {format(new Date(endDate), "dd/MM/yyyy")}
            </p>

            {/* Sales VAT Rates */}
            <h4 className="text-base font-medium mb-3">Sales VAT Rates</h4>
            ...
            {/* Purchase VAT Rates */}
            <h4 className="text-base font-medium mb-3 mt-8">
              Purchase VAT Rates
            </h4>
            <table className="w-full border-collapse mb-6">
              <thead>
            ...
            <div className="mt-6 p-3 border border-gray-300 bg-gray-50 text-sm">
              <p>
                <strong>Note:</strong> This report shows the breakdown of VAT by
                tax rates. Standard rate VAT applies to most goods and services,
                while reduced rate VAT applies to specific categories like
                essential goods, food items, or services as per local tax
                regulations. The transaction count represents the number of
                invoice line items with the respective tax rate.
              </p>
            </div>
          </div>
        </CardContent>
