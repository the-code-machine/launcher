...
import { openCreateForm } from '@/redux/slices/saleInvoiceFormSlice'
import {  useDeleteSaleInvoiceMutation } from '@/redux/api/saleInvoiceApi'
...
const SalesInvoicePage = () => {
  ...
  const [deleteInvoice, { isLoading: isDeleting }] = useDeleteSaleInvoiceMutation()
  ...
  const filteredInvoices = invoices?.filter((invoice) => {
    const matchesSearch = (
      (invoice.customer?.toLowerCase() || '').includes(filterInvoice.toLowerCase()) ||
      (invoice.invoiceNumber?.toLowerCase() || '').includes(filterInvoice.toLowerCase())
    )

    if (currentTab === 'all') return matchesSearch
    if (currentTab === 'cash' && invoice.saleType === 'cash') return matchesSearch
    if (currentTab === 'credit' && invoice.saleType === 'credit') return matchesSearch
  ...
  const calculateTotals = () => {
    ...
    invoices?.forEach(invoice => {
      ...
      totalPaid += invoice.receivedAmount || 0
                  ...
                  {isLoading ? (
                    ...
                    filteredInvoices?.map((invoice) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant={invoice.transactionType === 'cash' ? 'outline' : 'secondary'}
                            className={invoice.transactionType === 'cash' ? 'bg-green-50 text-green-700 border-green-200' : ''}
                          >
                            {invoice.transactionType === 'cash' ? 'Cash' : 'Credit'}
                          </Badge>
                        </TableCell>
        ...
        <Card className="w-full md:w-1/3">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle>
                {selectedInvoice ? (
                  <div className="flex items-center gap-2">
                    Invoice Details
                    <Badge
                      variant={selectedInvoice.saleType === 'cash' ? 'outline' : 'secondary'}
                      className={selectedInvoice.saleType === 'cash' ? 'bg-green-50 text-green-700 border-green-200' : ''}
                    >
                      {selectedInvoice.saleType === 'cash' ? 'Cash' : 'Credit'}
                    </Badge>
                  </div>
                ) : (
                  'Invoice Details'
             ...
             <Button onClick={()=> router.push(`/viewer?id=${selectedInvoice.id}`)}> Print</Button>
              )}
            </div>
          </CardHeader>
          <CardContent>
            {!selectedInvoice ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <Receipt className="h-16 w-16 text-gray-300 mb-2" />
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-semibold text-green-600">{formatCurrency(selectedInvoice.receivedAmount)}</p>
                      </div>
                      <div>
