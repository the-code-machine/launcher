...
import { openCreatePaymentOutForm } from '@/redux/slices/paymentSlice';
...
import { useGetPaymentOutsQuery } from '@/redux/api/paymentApi';
...
const PaymentOutPage = () => {
  ...
  const handleAddPayment = () => {
    dispatch(openCreatePaymentOutForm());
  };
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="Bone">All payment types</SelectItem>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <Card>
                    <CardContent className="p-2">
                      ...
                      <div className="font-medium">{formatCurrency(cashTotal)}</div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="p-2">
                      <div className="text-xs text-muted-foreground">Bank</div>
                      <div className="font-medium">{formatCurrency(bankTotal)}</div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="p-2">
                      <div className="text-xs text-muted-foreground">Cheque</div>
                      <div className="font-medium">{formatCurrency(chequeTotal)}</div>
                    </CardContent>
                  </Card>
                </div>
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoadingPayments ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                        {searchInput || selectedPartyId || paymentTypeFilter || dateRange.startDate || dateRange.endDate ? 
                      ...
                      <TableRow key={payment.id} className="cursor-pointer hover:bg-slate-50">
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={`
                              ${payment.paymentType === PaymentType.CASH ? 'bg-blue-50 text-blue-700 hover:bg-blue-50' : ''}
                              ${payment.paymentType === PaymentType.BANK ? 'bg-green-50 text-green-700 hover:bg-green-50' : ''}
                              ${payment.paymentType === PaymentType.CHEQUE ? 'bg-amber-50 text-amber-700 hover:bg-amber-50' : ''}
                            `}
                          >
