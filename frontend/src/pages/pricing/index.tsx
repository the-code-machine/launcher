...
import { Check, X, ArrowRight, Sparkles, Shield, Calendar, Star, Clock, AlertTriangle } from 'lucide-react';
...
export default function EnhancedPricingSection() {
  ...
  const hasActiveSubscription = (): boolean => {
    ...
    const isFree = userInfo.subscription.plan_name?.toLowerCase().includes('free');
  ...
  const getCurrentPlanId = (): number | null => {
    ...
    const matchingPlan = plans.find(plan => 
      plan.name.toLowerCase() === userInfo.subscription.plan_name.toLowerCase()
    );
  ...
  const handleSubscribe = async (plan: PlanType) => {
    ...
    if (userInfo?.subscription) {
      ...
      if (Number(plan.discounted_price) === 0) {
        alert("You must wait until your current subscription expires before switching to the free plan.");
        return;
      }
      ...
      if (hasActiveSubscription() && Number(plan.discounted_price) > 0) {
        alert("You already have an active subscription. You can only upgrade when your current plan expires.");
        return;
      }
    }
  ...
  const planFeatures = {
    free: [
      { feature: "Single user", included: true },
      { feature: "5 invoices per month", included: true },
      { feature: "Basic reporting", included: true },
      { feature: "Cloud backup", included: false },
    ...
    standard: [
      ...
      { feature: "Cloud backup", included: true },
    ...
    premium: [
      ...
      { feature: "Automatic backups", included: true },
      { feature: "24/7 premium support", included: true },
    ]
  };

  // Function to get features for a plan
  const getFeaturesForPlan = (planName: string) => {
    if (planName.toLowerCase().includes('free')) return planFeatures.free;
    if (planName.toLowerCase().includes('premium')) return planFeatures.premium;
    return planFeatures.standard;
  };
  ...
  const getSubscriptionStatus = (): { message: string; isActive: boolean; isExpired: boolean } => {
    if (!userInfo?.subscription) {
      return { message: "No active subscription", isActive: false, isExpired: false };
    }
    ...
    if (isExpired) {
      return { message: "Your subscription has expired", isActive: false, isExpired: true };
    }
    
    const timeLeft = getTimeRemaining();
    if (!timeLeft) return { message: "Active subscription", isActive: true, isExpired: false };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-600 mb-8 max-w-2xl mx-auto">
            Select the right plan for your business needs and start streamlining your invoicing process today.
          </p>

          {/* Current Subscription Status Banner */}
          {/* {userInfo?.subscription && (
            <div className={`mb-8 py-3 px-6 rounded-lg inline-flex items-center ${
              subscriptionStatus.isExpired ? 'bg-red-50 text-red-700' : 
              subscriptionStatus.isActive ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'
            }`}>
              <div className="mr-3">
              ...
              <div className="text-left">
                <div className="font-medium">
                  {userInfo.subscription.plan_name}
                </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {plans.map((plan, index) => {
              ...
              const discountPercentage = getDiscountPercentage(plan.price, plan.discounted_price);
              
              // Get icon for plan
              const PlanIcon = index === 0 ? Calendar : index === 1 ? Shield : Sparkles;
              ...
              return (
                <Card 
                  ...
                  className={`relative overflow-hidden transition-all duration-300 hover:shadow-xl ${
                    isActive ? 'border-green-500 bg-green-50/30 shadow-lg' :
                    isPopular ? 'border-red-500 shadow-lg transform hover:-translate-y-2' : 'hover:-translate-y-1'
                  }`}
                >
                  {isActive && (
                  ...
                  <CardHeader className={`pb-0 ${isPopular && !isActive ? 'bg-gray-50' : ''}`}>
                    <div className="flex items-center mb-3">
                      <div className={`p-2 rounded-lg ${
                        isActive ? 'bg-green-100 text-green-700' :
                        isPopular ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'
                      }`}>
                        <PlanIcon size={20} />
                      </div>
                    ...
                    <div className="mt-4">
                      {plan.price !== plan.discounted_price.toString() && (
                    ...
                    {plan.description && (
                      <p className="text-gray-600 text-sm mt-2 mb-5">{plan.description}</p>
                    )}
                  </CardHeader>
                        ...
                        <li key={i} className="flex items-start">
                          {item.included ? (
                            <Check className={`h-5 w-5 mr-3 ${
                              ...
                              isPopular ? 'text-red-500' : 'text-black'
                            }`} />
                          ) : (
                            <X className="h-5 w-5 mr-3 text-gray-300" />
                          )}
                          ...
                          <div className="w-full">
                            ...
                            >
                              {isActive 
                                ? 'Current Plan' 
                                : buttonDisabled 
                                  ? (isFree ? 'Unavailable' : 'Unavailable')
